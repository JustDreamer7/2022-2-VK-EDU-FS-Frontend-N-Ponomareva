...
export default function nonUniqueElements(data) {
  ...
  for (let i = 0; i < data.length; i++) {
    if (data.indexOf(data[i], i+1) != -1){
      index_dict[i] = 0
      index_dict[data.lastIndexOf(data[i])] = 0
    }
  }
  const sorted_index_list = Object.keys(index_dict).sort()
  sorted_index_list.forEach(function(key) {
    result_list.push(data[Number(key)])
    });
  return result_list
}
